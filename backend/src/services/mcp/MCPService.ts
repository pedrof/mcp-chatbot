...
import { GarminToolEnhancer } from './GarminToolEnhancer.js'
import { MCPToolResultValidator } from './MCPToolResultValidator.js'
...
export class MCPService {
  ...
  private garminEnhancer: GarminToolEnhancer
  private resultValidator: MCPToolResultValidator

  constructor(configService: ConfigService) {
    this.configService = configService
    this.garminEnhancer = new GarminToolEnhancer()
    this.resultValidator = new MCPToolResultValidator()
  }
  ...
  async executeTool(
    ...
    // Enhance arguments (e.g., auto-inject dates for Garmin tools)
    const enhancedArgs = this.garminEnhancer.enhanceArguments(toolName, args)

    try {
      console.log(`[MCPService] Calling tool '${toolName}' with arguments:`, JSON.stringify(enhancedArgs, null, 2))
      ...
      return this.resultValidator.validate(result, toolName)
    } catch (error: any) {
      console.error(`[MCPService] Error executing tool '${toolName}' on '${connection.serverName}':`)
      console.error(`  Error message: ${error.message}`)
      console.error(`  Error stack: ${error.stack}`)

      return this.resultValidator.createExecutionErrorResult(toolName, error)
    }
  }
